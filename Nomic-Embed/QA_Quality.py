...
def main():
    ...
    # Training
    dataframe_train = pd.read_parquet('Nomic-Embed/Dataset/train-hf-qa.parquet')
    #dataframe_train = dataframe_train.head(5)

    model = QA_Neural(dataframe_train, 0.1)

    # Validation
    dataframe_validation = pd.read_parquet('Nomic-Embed/Dataset/validation-hf-qa.parquet')  # Load Validation dataset
